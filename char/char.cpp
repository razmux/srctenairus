...
int char_mmo_char_tosql(uint32 char_id, struct mmo_charstatus* p){
	...
	if (
		...
		(p->show_equip != cp->show_equip) || (p->playtime != cp->playtime)
	)
	{	//Save status
		if( SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `base_level`='%d', `job_level`='%d',"
			...
			"`hotkey_rowshift`='%d', `clan_id`='%d', `title_id`='%lu', `show_equip`='%d',`playtime`='%u'"
			...
			p->playtime, p->account_id, p->char_id) )
		{
			Sql_ShowDebug(sql_handle);
			errors++;
		} else
...
int char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus* p, bool load_everything) {
	...
	if( SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT "
		...
		"`unban_time`,`font`,`uniqueitem_counter`,`sex`,`hotkey_rowshift`,`clan_id`,`title_id`,`show_equip`,`playtime`"
	...
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 62, SQLDT_UINT,   &p->playtime, 0, NULL, NULL)
