...
int party_share_loot(struct party_data* p, struct map_session_data* sd, struct item* item, int first_charid)
{
	...
	if (p && p->party.item&2 && (first_charid || !(battle_config.party_share_type&1))) {
		...
		if (battle_config.party_share_type&2) { // Round Robin
			...
			do {
				...
				if (pc_additem(psd,item,item->amount,LOG_TYPE_PICKDROP_PLAYER,true))
			...
			while (count > 0) { //Pick a random member.
				...
				if (pc_additem(psd[i],item,item->amount,LOG_TYPE_PICKDROP_PLAYER,true)) { // Discard this receiver.
					psd[i] = psd[count-1];
					count--;
	...
	if (!target) {
		...
		if ((i = pc_additem(sd,item,item->amount,LOG_TYPE_PICKDROP_PLAYER,true)))
			return i;
	}
...
int party_foreachsamemap(int (*func)(struct block_list*,va_list),struct map_session_data *sd,int range,...)
{
	...
	if(bg_mode) {
		if((bgd = bg_team_search(bg_team_get_id(&sd->bl))) == NULL)
			return 0;
	} else {
