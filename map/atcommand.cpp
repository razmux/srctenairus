...
ACMD_FUNC(mapmove)
{
	...
	{
		clif_displaymessage(sd->fd,"No puedes usar @go en PvPMode.");
		return -1;
	}
...
ACMD_FUNC(go)
{
	...
	const struct {
		...
		{ MAP_ARLANDRIA,   155, 192 }, //  37=Arlandria
	...
	if( battle_config.pvpmode_nowarp_cmd && sd->state.pvpmode )
	{
		clif_displaymessage(sd->fd,"No puedes usar @go mientras estás en PvPMode.");
		return -1;
	}
	...
	if (strncmp(map_name, "arlandria", 3) == 0 ||
		...
		town = 37;
		   ...
		   strncmp(map_name, "Zeny", 3) == 0) {
		town = 38;
	} else if (strncmp(map_name, "Mercado npc", 3) == 0 ||
		   strncmp(map_name, "npc", 3) == 0) {
		town = 39;
	}
...
ACMD_FUNC(mobinfo)
{
	...
	for (k = 0; k < count; k++) {
		unsigned int j,base_exp,job_exp,bonus;
		mob = mob_db(mob_ids[k]);
		base_exp = mob->base_exp;
		job_exp = mob->job_exp;

	     	if( pc_isPremium(sd) ){ // Muestra la experiencia incrementa en cuentas premium
			base_exp += (base_exp * battle_config.premium_base_exp_increase) / 100;
			job_exp += (job_exp * battle_config.premium_job_exp_increase) / 100;
		}
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			if ( pc_isPremium(sd)) // Muestra el aumento de drop rate de las cuentas premium
				droprate += (droprate * battle_config.premium_dropboost) / 100;
...
#endif
				if ( pc_isPremium(sd)) // Muestra el aumento de rate de items de las cuentas premium
					dropchance += (dropchance * battle_config.premium_dropboost) / 100;
...
ACMD_FUNC(rates)
{
	...
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1298), // Experience rates: Base %.2fx / Job %.2fx
		(battle_config.base_exp_rate + (pc_isPremium(sd) ? (battle_config.premium_base_exp_increase * battle_config.base_exp_rate) / 100 : 0)) / 100.,
		(battle_config.job_exp_rate + (pc_isPremium(sd) ? (battle_config.premium_job_exp_increase * battle_config.job_exp_rate) / 100 : 0)) / 100.);
	...
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1299), // Normal Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
		(battle_config.item_rate_common + (pc_isPremium(sd) ? (battle_config.premium_dropboost * battle_config.item_rate_common) / 100 : 0)) / 100.,
		(battle_config.item_rate_heal + (pc_isPremium(sd) ? (battle_config.premium_dropboost * battle_config.item_rate_heal) / 100 : 0)) / 100.,
		(battle_config.item_rate_use + (pc_isPremium(sd) ? (battle_config.premium_dropboost * battle_config.item_rate_use) / 100 : 0)) / 100.,
		(battle_config.item_rate_equip + (pc_isPremium(sd) ? (battle_config.premium_dropboost * battle_config.item_rate_equip) / 100 : 0)) / 100.,
		(battle_config.item_rate_card + (pc_isPremium(sd) ? (battle_config.premium_dropboost * battle_config.item_rate_card) / 100 : 0)) / 100.);
	clif_displaymessage(fd, buf);
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1300), // Boss Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
		(battle_config.item_rate_common_boss + (pc_isPremium(sd) ? (battle_config.premium_dropboost * battle_config.item_rate_common_boss) / 100 : 0)) / 100.,
		(battle_config.item_rate_heal_boss + (pc_isPremium(sd) ? (battle_config.premium_dropboost * battle_config.item_rate_heal_boss) / 100 : 0)) / 100.,
		(battle_config.item_rate_use_boss + (pc_isPremium(sd) ? (battle_config.premium_dropboost * battle_config.item_rate_use_boss) / 100 : 0)) / 100.,
		(battle_config.item_rate_equip_boss + (pc_isPremium(sd) ? (battle_config.premium_dropboost * battle_config.item_rate_equip_boss) / 100 : 0)) / 100.,
		(battle_config.item_rate_card_boss + (pc_isPremium(sd) ? (battle_config.premium_dropboost * battle_config.item_rate_card_boss) / 100 : 0)) / 100.);
	clif_displaymessage(fd, buf);
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1024), // MVP Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
		(battle_config.item_rate_common_mvp + (pc_isPremium(sd) ? (battle_config.premium_dropboost * battle_config.item_rate_common_mvp) / 100 : 0)) / 100.,
		(battle_config.item_rate_heal_mvp + (pc_isPremium(sd) ? (battle_config.premium_dropboost * battle_config.item_rate_heal_mvp) / 100 : 0)) / 100.,
		(battle_config.item_rate_use_mvp + (pc_isPremium(sd) ? (battle_config.premium_dropboost * battle_config.item_rate_use_mvp) / 100 : 0)) / 100.,
		(battle_config.item_rate_equip_mvp + (pc_isPremium(sd) ? (battle_config.premium_dropboost * battle_config.item_rate_equip_mvp) / 100 : 0)) / 100.,
		(battle_config.item_rate_card_mvp + (pc_isPremium(sd) ? (battle_config.premium_dropboost * battle_config.item_rate_card_mvp) / 100 : 0)) / 100.);
	clif_displaymessage(fd, buf);
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1301), // Other Drop Rates: MvP %.2fx / Card-Based %.2fx / Treasure %.2fx
		(battle_config.item_rate_mvp + (pc_isPremium(sd) ? (battle_config.premium_dropboost * battle_config.item_rate_mvp) / 100 : 0)) / 100.,
		(battle_config.item_rate_adddrop + (pc_isPremium(sd) ? (battle_config.premium_dropboost * battle_config.item_rate_adddrop) / 100 : 0)) / 100.,
		(battle_config.item_rate_treasure + (pc_isPremium(sd) ? (battle_config.premium_dropboost * battle_config.item_rate_treasure) / 100 : 0)) / 100.);
	clif_displaymessage(fd, buf);
...
ACMD_FUNC(duel)
{
	...
	if(sd->state.pvpmode) {
		clif_displaymessage(fd, "No puedes enviar duelo en PvPMode.");
		return 0;
	}
...
{
	if(sd->state.pvpmode) {
		clif_displaymessage(fd, "No puedes aceptar duelos en PvPMode.");
		return 0;
	}
	if(!duel_checktime(sd)) {
		char output[CHAT_SIZE_MAX];
...
ACMD_FUNC(myinfo)
{
	...
	//direct array initialization with variables is not standard C compliant.
		...
		output_table[4].value = pc_readaccountreg(sd,add_str("#DONAPOINTS"));
	...
	if( pc_isPremium(sd) )
	{
		...
		clif_displaymessage(fd,"- Cuenta Premium Activada -");
		atcommand_expinfo_sub(tick, &day, &hour, &minute, &second);
		sprintf(output, "La cuenta premium expira en : %d dias, %02d horas, %02d minutos, %02d segundos", day, hour, minute, second);
		clif_displaymessage(fd,output);
	}
 ...
 * @unequipall by [Cydh]
 * house.bad@gmail.com
 * Put off all equipped equipments
 *------------------------------------------*/
ACMD_FUNC(unequipall)
{
	...
	clif_displaymessage(fd, "Se te ha desequipado todo");
	return 0;
}
...
ACMD_FUNC(deleteall) {
	...
	if (sd->state.storage_flag != 1)
	{
		...
		if( storage_storageopen(sd) == 1 ) {
			clif_displaymessage(fd, "No puedes abrir el storage actualmente.");
			return -1;
		}
	}
...
ACMD_FUNC(show)
{
	...
	{
		clif_displaymessage(fd, "Usa: @show <x> <y>");
		return -1;
	}
...
ACMD_FUNC(heart) { //tylah
    ...
    nullpo_retr(-1, sd);
    if ( !message || !*message ) {
        clif_displaymessage(fd, "Usa: @heart 1 o 2");
        return -1;
    }
    if ( atoi(message) == 1 )
        clif_specialeffect(&sd->bl, 364,AREA);
    else if ( atoi(message) == 2 )
        clif_specialeffect(&sd->bl, 509,AREA);
    return 0;
}
...
ACMD_FUNC(dance) {
   ...
   nullpo_retr(-1, sd);
	...
	if (!message || !*message || atoi(message) > 9 || atoi(message) < 1 ) {
		clif_displaymessage(fd, "Usa: @dance 1-9");
		return -1;
	}
	switch ( atoi(message) ) {
		case 1: clif_specialeffect(&sd->bl, 413,AREA); break;
		case 2: clif_specialeffect(&sd->bl, 414,AREA); break;
		case 3: clif_specialeffect(&sd->bl, 415,AREA); break;
		case 4: clif_specialeffect(&sd->bl, 426,AREA); break;
		case 5: clif_specialeffect(&sd->bl, 458,AREA); break;
		case 6: clif_specialeffect(&sd->bl, 466,AREA); break;
		case 7: clif_specialeffect(&sd->bl, 501,AREA); break;
		case 8: clif_specialeffect(&sd->bl, 540,AREA); break;
		case 9: clif_specialeffect(&sd->bl, 550,AREA); break;
	}
   return 0;
}
...
ACMD_FUNC(allchat) 
{
	...
	if (!message || !*message || sscanf(message, "%199[^\n]", tempmes) < 0) {
		clif_displaymessage(fd, "Porfavor inserta el mensaje (Usa: @allchat <message>).");
		return -1;
	}
...
ACMD_FUNC(allemo) {
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Usa: @emo 1-81");
	return -1;
   }
 ...
 * Turn On/Off autostore for player
 * @author [Cydh]
 **/
ACMD_FUNC(autostore) {
	uint8 atype = 0, aweight = 0;

	nullpo_retr(-1, sd);

	if (sscanf(message, "%3hhu %3hhu", &atype, &aweight) > 0) {
		atype = cap_value(atype,1,15);

		clif_displaymessage(fd, "Autostoring item:");
		if (atype&1) clif_displaymessage(fd, "-> Usable/healing items");
		if (atype&2) clif_displaymessage(fd, "-> Equipments");
		if (atype&4) clif_displaymessage(fd, "-> Etc and misc");
		if (atype&8) clif_displaymessage(fd, "-> Cards");

		if (aweight) {
			aweight = cap_value(aweight,0,100);
			sprintf(atcmd_output, "Autostore the item when weight is >= %hhu%%.", aweight);
			clif_displaymessage(fd, atcmd_output);
		}

		sd->autostore_type = atype;
		sd->autostore_weight = aweight;
		sd->state.autostore = 1;

		clif_displaymessage(fd, "Autostore is ACTIVE now.");
	}
	else {
		if (sd->state.autostore) {
			sd->state.autostore = 0;
			//sd->autostore_type = 0;
			//sd->autostore_weight = 0;
			clif_displaymessage(fd, "Autostore has been PAUSED.");
			return 0;
		}
		clif_displaymessage(fd, "Usage: @autostore <item_type> [<min_weight>]");
		clif_displaymessage(fd, "<item_type>: Type that will be autostore (using bitmask)");
		clif_displaymessage(fd, " 1: Usable/healing items");
		clif_displaymessage(fd, " 2: Equipments");
		clif_displaymessage(fd, " 4: Etc and misc");
		clif_displaymessage(fd, " 8: Cards");
		clif_displaymessage(fd, "<min_weight>: (Optional) Minimum weight to active autostore, default to 0");
		clif_displaymessage(fd, "Examples:");
		clif_displaymessage(fd, " @autostore 2    => Equipments only");
		clif_displaymessage(fd, " @autostore 9    => Usable items and cards");
		clif_displaymessage(fd, " @autostore 15   => All items");
		clif_displaymessage(fd, " @autostore 1 50 => Autostore usable/healing items if weight >= 50%");
		if (!sd->state.autostore && sd->autostore_type) {
			sd->state.autostore = 1;
			clif_displaymessage(fd, "Autostore is REACTIVATED using previous settings.");
			return 0;
		}
		return -1;
	}
...
ACMD_FUNC(battlestats)
{
    ...
    buf.append("\n");
    buf.append("   -> ATK : ");
    buf.append(std::to_string(pc_leftside_atk(sd) + pc_rightside_atk(sd)));
    buf.append("\n");
    buf.append("   -> MATK : ");
    buf.append(std::to_string(pc_leftside_matk(sd) + pc_rightside_matk(sd)));
    buf.append("\n");
    buf.append("   -> HIT : ");
    buf.append(std::to_string(status->hit));
    buf.append("\n");
    buf.append("   -> CRIT : ");
    buf.append(std::to_string(status->cri / 10));
    buf.append("\n");
    buf.append("   -> DEF : ");
    buf.append(std::to_string(pc_leftside_def(sd) + pc_rightside_def(sd)));
    buf.append("\n");
    buf.append("   -> MDEF : ");
    buf.append(std::to_string(pc_leftside_mdef(sd) + pc_rightside_mdef(sd)));
    buf.append("\n");
    buf.append("   -> FLEE : ");
    buf.append(std::to_string(status->flee + status->flee2 / 10));
    buf.append("\n");
    buf.append("   -> ASPD : ");
    buf.append(std::to_string((2000 - status->amotion) / 10));
    buf.append("\n");
    buf.append("   -> Base Lv. ");
    ...
    buf.append("   -> Base ATK. ");
    buf.append(std::to_string(sd->battle_status.batk + sd->battle_status.rhw.atk + sd->battle_status.lhw.atk));
    buf.append("\n");
    ...
    buf.append("|--- Bonus Physical Damages ---|");
	if (sd->right_weapon.addsize[SZ_MEDIUM] + sd->left_weapon.addsize[SZ_MEDIUM]) {
        buf.append("   -> Medium Size: ");
        buf.append(std::to_string(sd->right_weapon.addsize[SZ_MEDIUM] + sd->left_weapon.addsize[SZ_MEDIUM]));
        buf.append("\n");
	}
	if (sd->right_weapon.addsize[SZ_ALL] + sd->left_weapon.addsize[SZ_ALL]) {
        buf.append("   -> All Sizes: ");
        buf.append(std::to_string(sd->right_weapon.addsize[SZ_ALL] + sd->left_weapon.addsize[SZ_ALL]));
        buf.append("\n");
	}
	if (sd->right_weapon.addrace[RC_PLAYER] + sd->left_weapon.addrace[RC_PLAYER]) {
        buf.append("   -> Player: ");
        buf.append(std::to_string(sd->right_weapon.addrace[RC_PLAYER] + sd->left_weapon.addrace[RC_PLAYER]));
        buf.append("\n");
	}
	if (sd->right_weapon.addrace[RC_ALL] + sd->left_weapon.addrace[RC_ALL]) {
        buf.append("   -> All Races: ");
        buf.append(std::to_string(sd->right_weapon.addrace[RC_ALL] + sd->left_weapon.addrace[RC_ALL]));
        buf.append("\n");
	}
	if (sd->right_weapon.addclass[CLASS_NORMAL] + sd->left_weapon.addclass[CLASS_NORMAL]) {
        buf.append("   -> Normal Class: ");
        buf.append(std::to_string(sd->right_weapon.addclass[CLASS_NORMAL] + sd->left_weapon.addclass[CLASS_NORMAL]));
        buf.append("\n");
	}
	if (sd->right_weapon.addclass[CLASS_ALL] + sd->left_weapon.addclass[CLASS_ALL]) {
        buf.append("   -> All Classes: ");
        buf.append(std::to_string(sd->right_weapon.addclass[CLASS_ALL] + sd->left_weapon.addclass[CLASS_ALL]));
        buf.append("\n");
	}

        buf.append("\n");

    buf.append("|--- Bonus Magic Damages ---|");
	if (sd->magic_addsize[SZ_MEDIUM]) {
        buf.append("   -> Medium Size: ");
        buf.append(std::to_string(sd->magic_addsize[SZ_MEDIUM]));
        buf.append("\n");
	}
	if (sd->magic_addsize[SZ_ALL]) {
        buf.append("   -> All Sizes: ");
        buf.append(std::to_string(sd->magic_addsize[SZ_ALL]));
        buf.append("\n");
	}
	if (sd->magic_addrace[RC_PLAYER]) {
        buf.append("   -> Player: ");
        buf.append(std::to_string(sd->magic_addrace[RC_PLAYER]));
        buf.append("\n");
	}
	if (sd->magic_addrace[RC_ALL]) {
        buf.append("   -> All Races: ");
        buf.append(std::to_string(sd->magic_addrace[RC_ALL]));
        buf.append("\n");
	}
	if (sd->magic_addclass[CLASS_NORMAL]) {
        buf.append("   -> Normal Class: ");
        buf.append(std::to_string(sd->magic_addclass[CLASS_NORMAL]));
        buf.append("\n");
	}
	if (sd->magic_addclass[CLASS_ALL]) {
        buf.append("   -> All Classes: ");
        buf.append(std::to_string(sd->magic_addclass[CLASS_ALL]));
        buf.append("\n");
	}
        buf.append("\n");
    ...
    buf.append("|--- Race Resistance ---|\n"); // Corregir formato antiguo 
	struct {
		const char *format;
		int value;
	} race_resist_info[] = {
		{ "   -> Formless: %d%%", sd->subrace[RC_FORMLESS] },
		{ "   -> Undead: %d%%", sd->subrace[RC_UNDEAD] },
		{ "   -> Beast: %d%%", sd->subrace[RC_BRUTE] },
		{ "   -> Plant: %d%%", sd->subrace[RC_PLANT] },
		{ "   -> Insect: %d%%", sd->subrace[RC_INSECT] },
		{ "   -> Fish: %d%%", sd->subrace[RC_FISH] },
		{ "   -> Demon: %d%%", sd->subrace[RC_DEMON] },
		{ "   -> DemiHuman: %d%%", sd->subrace[RC_DEMIHUMAN] },
		{ "   -> Angel: %d%%", sd->subrace[RC_ANGEL] },
		{ "   -> Dragon: %d%%", sd->subrace[RC_DRAGON] },
		{ "   -> Player: %d%%", sd->subrace[RC_PLAYER] },
		{ "   -> ALL: %d%%", sd->subrace[RC_ALL] },
		{ "   -> Normal Class: %d%%", sd->subclass[CLASS_NORMAL] },
	};
	for (int i = 0; i < ARRAYLENGTH(race_resist_info); i++) {
		if (race_resist_info[i].value == 0)
            ...
            buf.append("      ");
            buf.append(race_resist_info[i].format);
            buf.append(" = ");
            buf.append(std::to_string(race_resist_info[i].value));
            buf.append("%\n");
	}
    buf.append("|--- Elemental Resistances ---|\n");
    for (int8 type = ELE_NEUTRAL; type < ELE_ALL; type++)
    {
        int resistance = get_ele_fix(sd, type);
        if (resistance > 0)
        {
 	    buf.append("   -> ");
            buf.append(elements[type]);
            buf.append(" = ");
	...
	int id = 0;
        ...
        if (skill_db[it.id] != nullptr)
        {
            buf.append("      Increase the damage of '");
            buf.append(skill_db[it.id]->desc);
            buf.append("' by ");
    ...
    if (!sd->state.lr_flag || sd->state.lr_flag == 3)
        ...
        atkPercentage += sd->arrow_addclass[CLASS_ALL];
    buf.append("   -> ATK (%) : ");
    ...
    buf.append("%  -> ASPD (%) : ");
    buf.append(std::to_string(pc_readparam(sd, SP_ASPD_RATE)/ 10));
    buf.append("%\n");
    buf.append("''ASPD (%) No se basa en el valor final sino en la ecuación.''\n\n");
    ...
    buf.append("--------------------------------------------------------\n");
    buf.append("INCREASES HEAL (%) : ");
    buf.append(std::to_string(sd->bonus.add_heal_rate));
    buf.append("% / ");
    ...
    buf.append("--------------------------------------------------------\n");
    buf.append("FIXED CAST TIME (%) : ");
    buf.append(std::to_string(sd->bonus.fixcastrate));
    buf.append("%\n");
    buf.append("FIXED CAST TIME (S)  : ");
    buf.append(std::to_string((sd->bonus.add_fixcast / 1000)));
    buf.append("s\n");
    buf.append("SKILL DELAY (%)         : ");
    buf.append(std::to_string((sd->delayrate - 100)));
    buf.append("%\n");
...
ACMD_FUNC(autorefresh)
{
	...
	if( !message || !*message )
		clif_displaymessage(fd,"Por favor, ingrese un valor entre 30 y 300 segundos o desactívelo para deshabilitarlo. Uso @autorefresh <segundos/off>");
	else if( !strcmpi(message,"off") )
	{
		if( sd->sc.data[SC_AUTOREFRESH] )
		{
			status_change_end(&sd->bl,SC_AUTOREFRESH,INVALID_TIMER);
			clif_displaymessage(fd,"-- AutoRefresh desacticado --");
			return 0;
		}

		clif_displaymessage(fd,"AutoRefresh no está habilitado.");
	}
	else if( (time = atoi(message)) < 30 || time > 300 )
		clif_displaymessage(fd,"Por favor, ingrese un valor entre 30 y 300 segundos");
	else
	{
		if( sd->sc.data[SC_AUTOREFRESH] )
			sprintf(atcmd_output,"Ajuste de AutoRefresh para que se ejecute cada %d segundos.",time);
		else
			sprintf(atcmd_output,"Iniciando AutoRefresh para ejecutar cada %d segundos.",time);
...
static int atwhopk_sub(struct block_list *bl,va_list ap)
{
	...
	if( fd )
	{
		...
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN],pl_info, false, SELF);
...
{
	clif_aura_reload();
       clif_displaymessage(fd, "Se ha recargado la base de datos de Aura.");
	return 0;
}
...
ACMD_FUNC(changebg) 
{
	...
	{
		clif_displaymessage(fd, "Fall?? se encontr??C");
		return -1;
	}
}
...
ACMD_FUNC(orderbg) 
{
	...
	if (!message || !*message || sscanf(message, "%d %d", &poc, &bgid) < 2) 
	{
		...
		clif_displaymessage(fd,"6 - Poring Ball");

		clif_displaymessage(fd,"-----------------------------------------------");
		clif_displaymessage(fd, "Usa: @orderbg <position> <bg_id>.");
		return -1;
	}

	if ( poc < 1 || poc > MAX_BG_ARRAY )
	{
		clif_displaymessage(fd, "Usage: @orderbg <position (1 to 6)> <bg_id>.");
		return -1;
	}
	
	if ( bgid < 1 || bgid > MAX_BG_ARRAY )
	{
		clif_displaymessage(fd, "Usage: @orderbg <position> <bg_id (1 to 6)>.");
		return -1;
	}
...
ACMD_FUNC(bgpoints) 
{
	...
	clif_displaymessage( fd,"[Battle Ground]: Puntos actualizados, Usa @myinfo para mas información");
	return 0;
}
