...
#include "mapreg.hpp"
#include "oboro.hpp" // struct s_vending
...
void pc_reg_received(struct map_session_data *sd)
{
	...
	sd->Premium_Tick = pc_readaccountreg(sd,add_str("#Premium_Tick")); // Premium Account System
       // @Aura
       sd->user_aura = pc_readglobalreg(sd, add_str("USERAURA")); // Aura System eAmod
	...
	if( pc_isPremium(sd) )
	{
		int tick = sd->Premium_Tick - (int)time(NULL), day, hour, minute, second, gm_lvl = pc_get_group_level(sd);
	...
	{
		int erate = battle_config.bg_event_extra_badges;
 ...
 *   7 = stack limitation
 */
enum e_additem_result pc_additem(struct map_session_data *sd,struct item *item,int amount,e_log_pick_type log_type, bool autostoring) {
	struct item_data *id;
	int16 i;
	unsigned int w;
	...
	if (autostoring && sd->state.autostore && !item->expire_time && !id->flag.autoequip && !sd->state.storage_flag && !sd->npc_id) {
			...
			)
		{
			...
			sprintf(output, "[ Autostore ] %dx %s", amount, id->jname);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
...
static void pc_calcexp(struct map_session_data *sd, unsigned int *base_exp, unsigned int *job_exp, struct block_list *src)
{
	...
	{
		int rank;
		if( (rank = pc_famerank(sd->status.char_id,-3)) && rand()%100 < ((MAX_FAME_LIST + 1 - rank) * 4) )
...
void pc_damage(struct map_session_data *sd,struct block_list *src,unsigned int hp, unsigned int sp)
{
	...
	if( &sd->sc && sd->sc.data[SC_ALL_RIDING] && battle_config.unmount_on_damage && !pc_isPremium(sd) ) // [DarbladErxX] desmontar al recibir daño
	status_change_end(&sd->bl, SC_ALL_RIDING, INVALID_TIMER); //release mount


	if( sd->state.pvpmode && src->type == BL_PC && ((TBL_PC*)src)->state.pvpmode ) // PvPmode eAmod 
	{
...
int pc_get_aura(struct map_session_data *sd)
{
	...
	if( i < MAX_FAME_LIST )
		return battle_config.aura_top10_bg; // Default 4
...
void pc_pvpmode(struct map_session_data* sd)
{
	...
	if( !map_getmapflag(sd->bl.m, MF_NOPVPMODE) && sd->idlepvp >= last_tick - battle_config.pvpmode_enable_delay )
	{
		char pkinfo[100];
		unsigned int tick = (unsigned int)(sd->idlepvp - last_tick + battle_config.pvpmode_enable_delay);
		sprintf(pkinfo, "[Modo PK deshabilitado - Espera %d segundos para entrar de nuevo en modo PK.]", tick);
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], pkinfo, false, SELF);
		return;
	}
	...
	clif_displaymessage(sd->fd, "[Ingresaste al modo PK: puedes matar / ser asesinado por otros en el modo PK]");
	return;
}
...
void pc_pvpmodeoff(struct map_session_data* sd, short force, short flag)
{
	...
	if( !force && sd->idlepvp >= last_tick - battle_config.pvpmode_disable_delay )
	{
		...
		sprintf(pkinfo, "[PK habilitado - %d segundos para desactivarlo]", tick);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], pkinfo, false, SELF);
		return;
	}
