...
inline bool mapdata_nopvpmode(struct map_data *mapdata) {
	...
	if(mapdata->flag[MF_GVG] || mapdata->flag[MF_PVP] || mapdata->flag[MF_NOPVPMODE] || mapdata->flag[MF_GVG_CASTLE] || mapdata->flag[MF_BATTLEGROUND] )
		return true;

	return false;
}

inline bool mapdata_noguildwar(struct map_data *mapdata) {
	if (mapdata == nullptr)
		return false;
	
	if(mapdata->flag[MF_TOWN] || mapdata->flag[MF_GVG] || mapdata->flag[MF_PVP] || mapdata->flag[MF_NOGUILDWAR] || mapdata->flag[MF_GVG_CASTLE] || mapdata->flag[MF_BATTLEGROUND] )
		return true;
...
inline bool map_nopvpmode(int16 m) {
	if (m < 0)
		return false;

	struct map_data *mapdata = &map[m];

	return mapdata_nopvpmode(mapdata);
}

inline bool map_noguildwar(int16 m) {
	if (m < 0)
		return false;

	struct map_data *mapdata = &map[m];

	return mapdata_noguildwar(mapdata);
}
