...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv)
{
	...
	//PK damage rates
	if( sd && damage && ((battle_config.pk_mode && map_getmapflag(bl->m, MF_PVP)) || (s_bl && s_bl->type == BL_PC && (guild_wardamage(sd) || (((TBL_PC *)src)->state.pvpmode && battle_config.pvpmode_gvgreductions)))) ) {
...
static void battle_calc_weapon_final_atk_modifiers(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
		...
		)
	{
		...
		status_fix_damage(target,src,wd->damage,clif_damage(target,src,gettick(),0,0,rejected,0,DMG_NORMAL,0,false),ST_REJECTSWORD);
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	switch( t_bl->type ) { //Checks on target master
		case BL_PC: {
			...
			if( sd->state.pvpmode && s_bl->type == BL_PC && !((TBL_PC*)s_bl)->state.pvpmode && flag&BCT_ENEMY && s_bl->type != BL_MOB )
	...
	{	//Checks on source master
		case BL_PC:
		{
			...
			else if( sd->state.pvpmode && s_bl->type == BL_PC && !((TBL_PC*)s_bl)->state.pvpmode && s_bl->type != BL_MOB )
				return 0; 
			}
	...
	{ //Non pvp/gvg, check party/guild settings.
		if( battle_config.guild_wars && !map_noguildwar(m) && !(agit_flag || agit2_flag) )
		{
			int s_guild = status_get_guild_id(s_bl);
			int t_guild = status_get_guild_id(t_bl);
