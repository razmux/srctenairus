...
void initChangeTables(void)
{
	...
	// StatusIconChangeTable[SC_INCFLEE2] = EFST_PLUSAVOIDVALUE; -- Removed to use this Status Icon on AutoRefresh feature
	//eAmod
	StatusIconChangeTable[SC_AUTOREFRESH] = EFST_PLUSAVOIDVALUE;
...
//		if (status_get_race2(bl) == RC2_GVG && status_sc2scb_flag(type)&SCB_MAXHP) return 0;
	...
	switch(type)
	{
       ...
       if (sd && sd->view_aura > 0 && (type == SC_HIDING || type == SC_CLOAKING || type == SC_CHASEWALK || type == SC_CLOAKINGEXCEED || type == SC__INVISIBILITY || type == SC__SHADOWFORM)) {
               sd->view_aura *= -1;
...
TIMER_FUNC(status_change_timer){
	...
	case SC_AUTOREFRESH: // eAmod
		if( sd )
		{
			...
			else if( sd->bl.prev && ( map_getmapflag(sd->bl.m, MF_BATTLEGROUND) || map_getmapflag(sd->bl.m, MF_GVG) ) )
			{
				sce->val2 = 1;
				clif_refresh(sd);
				sce->val2 = 0;
