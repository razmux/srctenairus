...
static bool clif_parse_row_aura(char* split[], int columns, int current)
{
	...
	if( aura_id <= 0 )
	{
		ShowError("clif_parse_row_aura : ID de aura no válida %d\n", aura_id);
		return false;
	}
	...
	case BL_PC:
		{
			...
			if( (battle_config.bg_eAmod_mode || battle_config.force_send_bgemblem ) && sd->bg_id && map_getmapflag(sd->bl.m, MF_BATTLEGROUND) )
	...
	case BL_PC:
		{
			...
			if( ( battle_config.bg_eAmod_mode || battle_config.force_send_bgemblem ) && tsd->bg_id && map_getmapflag(tsd->bl.m, MF_BATTLEGROUND) )
...
void clif_map_property(struct block_list *bl, enum map_property property, enum send_target t)
{
	...
	WBUFL(buf,4) = ((mapdata->flag[MF_PVP]?1:0)<<0)| // PARTY - Show attack cursor on non-party members (PvP)
	       (((mapdata->flag[MF_BATTLEGROUND] && !battle_config.no_gvg_on_bg ) || mapdata_flag_gvg2(mapdata)?1:0)<<1)|// GUILD - Show attack cursor on non-guild members (GvG)
	       (((mapdata->flag[MF_BATTLEGROUND] && !battle_config.no_siege_on_bg ) || mapdata_flag_gvg2(mapdata)?1:0)<<2)|// SIEGE - Show emblem over characters heads when in GvG (WoE castle)
...
int clif_nodelay (struct block_list *src)
{
	...
	clif_send (buf, packet_len (0x9cb), src, SELF);
...
void clif_guild_basicinfo(struct map_session_data *sd) {
	...
	if(bgd != NULL) {
		...
		if((bg_sd = bgd->members[0].sd) == NULL)
...
bool clif_name2(struct block_list *src, struct block_list *bl, send_target target )
{
	...
	switch(bl->type) {
		case BL_MOB:
			safestrncpy(WBUFCP(buf,10), ((TBL_MOB*)bl)->name, NAME_LENGTH);
			break;
		case BL_HOM:
			safestrncpy(WBUFCP(buf,10), ((TBL_HOM*)bl)->homunculus.name, NAME_LENGTH);
			break;
		case BL_MER:
			safestrncpy(WBUFCP(buf,10), ((TBL_MER*)bl)->db->name, NAME_LENGTH);
			break;
		case BL_PET:
			safestrncpy(WBUFCP(buf,10), ((TBL_PET*)bl)->pet.name, NAME_LENGTH);
			break;
		case BL_NPC:
			safestrncpy(WBUFCP(buf,10), ((TBL_NPC*)bl)->name, NAME_LENGTH);
			break;
		case BL_ELEM:
			safestrncpy(WBUFCP(buf,10), ((TBL_ELEM*)bl)->db->name, NAME_LENGTH);
			break;
		default:
			ShowError("clif_name2: bad type %d(%d)\n", bl->type, bl->id);
	}
	struct unit_data *ud = unit_bl2ud(bl);
	if (ud->groupId)
		WBUFL(buf,6) = ud->groupId;
	else
		WBUFL(buf,6) = 0;
	if (ud->title)
		safestrncpy(WBUFCP(buf,34), ud->title, NAME_LENGTH);
	else
		WBUFB(buf,34) = 0;
	...
	case BL_MOB:
		{
			...
			else if( md->bg_id && (bgd = bg_team_search(md->bg_id)) != NULL && (g = bgd->g) != NULL )
			{
				WBUFW(buf,0) = cmd = 0x195;
				WBUFB(buf,30) = 0;
...
void clif_parse_LoadEndAck(int fd,struct map_session_data *sd)
{
	...
	if( sd->state.pvpmode && map_nopvpmode(sd->bl.m) )
